...
if command -v jq &>/dev/null; then
    ...
    REPO=$(echo "$INPUT" | jq -r '.config.repo // empty')
    STRIP_PREFIX=$(echo "$INPUT" | jq -r '.config["strip-prefix"] // "v"')
    GITHUB_TOKEN=$(echo "$INPUT" | jq -r '.config["github-token"] // empty')
    API_URL=$(echo "$INPUT" | jq -r '.config["api-url"] // "https://api.github.com"')
else
